...
"""
    ...
    def get_job_description_from_db(self, template_id: str = None) -> tuple[str, str]:
        ...
        try:
            ...
            return job_data["user_job_description"], job_data["template_id"]
    ...
    def save_initial_json_to_db(self, template_id: str, initial_json: Dict[str, Any]):
        """Save the initial processed JSON to database"""
        if not self.supabase_client:
            raise Exception("Supabase client not initialized")
        ...
        try:
            ...
            logger.info(f"Initial JSON saved to database for template ID: {template_id}")
            return result.data
            
        except Exception as e:
            logger.error(f"Error saving initial JSON to database: {e}")
            raise
    
    def save_final_schema_to_db(self, template_id: str, final_schema: Dict[str, Any]):
        """Save the final JSON schema to database"""
        if not self.supabase_client:
            raise Exception("Supabase client not initialized")
        
        try:
            result = self.supabase_client.table("job_templates").update({
                "final_json_schema": final_schema
            }).eq("template_id", template_id).execute()
            
            logger.info(f"Final JSON schema saved to database for template ID: {template_id}")
            return result.data
            
        except Exception as e:
            logger.error(f"Error saving final schema to database: {e}")
            raise
    ...
    def process_job_description_from_db(self, template_id: str = None, template_path: str = None, save_local: bool = True) -> Dict[str, Any]:
        """Main method to process job description from database and save results back to database"""
        try:
            # Test connections
            ...
            job_description, db_template_id = self.get_job_description_from_db(template_id)
            ...
            # Save initial template to database
            logger.info("Saving initial JSON template to database...")
            self.save_initial_json_to_db(db_template_id, template)
            ...
            # Save final schema to database
            logger.info("Saving final JSON schema to database...")
            self.save_final_schema_to_db(db_template_id, filled_template)
            ...
            if save_local:
                ...
                output_filename = f"filled_job_template_{db_template_id}_{timestamp}.json"
...
def process_single_job(template_id: str = None):
    ...
    try:
        ...
        print("\n=== Processing Complete ===")
        print(f"Job ID: {result.get('jobProfile', {}).get('JobId', 'N/A')}")
